...
A test that holds for one value at a time, such as `StartsWith`, is refused: it needs the field read position by position, which the functions above do not do. So is membership that every value must pass, `All(t => wanted.Contains(t))`, or that some value must fail, `Any(t => !wanted.Contains(t))`: `MATCH` answers whether some value is one of them, and `!p.Tags.Any(t => wanted.Contains(t))` asks that none is.
